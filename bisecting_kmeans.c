...
void assignInitData(int dim, int ndata, double *data, int k, int *cluster_size, int *cluster_start, double **cluster_centroid, int *cluster_assign, int clust1, int clust2) { //ndata does not refer to whole data set, only to amount in the specified cluster
	int i, j, l, cent_idx, temp_data_idx = 0, temp_assign_idx = 0, clust_size_idx_cnt = cluster_start[clust1], range, temp_size = 0;
		...
		for (i =cluster_start[clust1]; i < cluster_start[clust1]+ndata*dim; i+=dim) { //for each data point in the cluster original cluster	
			...
			if (cluster_assign[i/dim] == l) {
				...
				for (j = 0; j < dim; j++) {
                    ...
                    temp_data[temp_data_idx+j] = data[i+j];
				}	
				temp_assign[temp_assign_idx] = l;
				temp_data_idx+=dim;
                temp_assign_idx++;
			}
		}
    ...
    temp_data_idx = 0;
    temp_assign_idx = 0;
	for (i = cluster_start[clust1]; i < cluster_start[clust1]+ndata*dim; i+=dim) {
		cluster_assign[i/dim] = temp_assign[temp_assign_idx];
        for (j = 0; j < dim; j++) {
            data[i+j] = temp_data[temp_data_idx+j];
        }
        temp_assign_idx++;
        temp_data_idx += dim;
		//printf("%d,", cluster_assign[i]);
...
int bkInitCentroids(int dim, int ndata, double *data, int *cluster_size, int *cluster_start, double **cluster_centroid, int place_idx, int cent_idx, int cent_count, int *cluster_assign) {
	int i, j, l, centroid_point, sums_idx;
	...
	for (j = cluster_start[cent_idx]; j < cluster_start[cent_idx]+cluster_size[cent_idx]*dim; j+=dim) { //for each data point in the cluster
        ...
        distance = pnt2centDistance(dim, cent_idx, j, data, cluster_centroid);
	...
	assignInitData(dim, cluster_size[cent_idx], data, 2, cluster_size, cluster_start, cluster_centroid, cluster_assign, cent_idx, place_idx);
	...
	for (i = 0; i < cent_count; i++) { //for each cluster
		for (l = cluster_start[i]; l < cluster_start[i]+cluster_size[i]*dim; l+=dim) {
            ...
            sums_hold = pnt2centDistance(dim, i, l, data, cluster_centroid);
